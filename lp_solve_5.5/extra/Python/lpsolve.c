...
static int create_handle(structlpsolve *lpsolve, lprec *lp0, char *err)
{
	...
	for (i = 0; (i <= lp_last) && (lpdata[i].lp != NULL); i++);
	if (i > lp_last) {
	  	i = ++lp_last;
                if ((i % LPSTEP) == 0) {
#if defined FORTIFY
...
#endif
                        if (i == 0)
                                lpdata = (struct lpdata *) malloc(LPSTEP * sizeof(*lpdata));
                        else
                                lpdata = (struct lpdata *) realloc(lpdata, (i + LPSTEP) * sizeof(*lpdata));
#if defined FORTIFY
                        Fortify_Disable(-2);
#endif
                        memset(lpdata + i, 0, LPSTEP * sizeof(*lpdata));
                }
        }
        lpdata[i].lp = lp0;
        lpdata[i].newbranch = NULL;
        lpdata[i].newbranchhandle = NULL;
        lpdata[i].newnode = NULL;
        lpdata[i].newnodehandle = NULL;
        lpdata[i].newabort = NULL;
        lpdata[i].newaborthandle = NULL;
        lpdata[i].newlog = NULL;
        lpdata[i].newloghandle = NULL;
        lpdata[i].newmsg = NULL;
        lpdata[i].newmsghandle = NULL;
...
{
        if (handle < 0 || handle > lp_last || lpdata[handle].lp == NULL)
	   return(0);
        else
           return(1);
}
...
static void delete_handle(int handle)
{
        if (handle_valid(handle)) {
                ...
                delete_lp(lp0);
                lpdata[handle].lp = NULL;
                FreeFunction(lpdata[handle].newbranch);
                FreeFunction(lpdata[handle].newbranchhandle);
                FreeFunction(lpdata[handle].newnode);
                FreeFunction(lpdata[handle].newnodehandle);
                FreeFunction(lpdata[handle].newabort);
                FreeFunction(lpdata[handle].newaborthandle);
                FreeFunction(lpdata[handle].newlog);
                FreeFunction(lpdata[handle].newloghandle);
                FreeFunction(lpdata[handle].newmsg);
                FreeFunction(lpdata[handle].newmsghandle);
                lpdata[handle].newbranch = NULL;
                lpdata[handle].newbranchhandle = NULL;
                lpdata[handle].newnode = NULL;
                lpdata[handle].newnodehandle = NULL;
                lpdata[handle].newabort = NULL;
                lpdata[handle].newaborthandle = NULL;
                lpdata[handle].newlog = NULL;
                lpdata[handle].newloghandle = NULL;
                lpdata[handle].newmsg = NULL;
                lpdata[handle].newmsghandle = NULL;
        }
}
...
/* xxlpsolve('put_bb_nodefunc', lp, newnode, nodehandle) */

static void impl_put_bb_nodefunc(structlpsolve *lpsolve)
{
        Check_nrhs(lpsolve, 3);
        FreeFunction(lpsolve->newnode);
        FreeVariant(lpsolve->newnodehandle);
        lpsolve->newnode = GetFunction(&lpsolve->lpsolvecaller, 2);
        lpsolve->newnodehandle = GetVariant(&lpsolve->lpsolvecaller, 3);
        put_bb_nodefunc(lpsolve->lp, nodefunction, lpsolve);
}


/* xxlpsolve('put_bb_branchfunc', lp, newbranch, branchhandle) */

static void impl_put_bb_branchfunc(structlpsolve *lpsolve)
{
        Check_nrhs(lpsolve, 3);
        FreeFunction(lpsolve->newbranch);
        FreeVariant(lpsolve->newbranchhandle);
        lpsolve->newbranch = GetFunction(&lpsolve->lpsolvecaller, 2);
        lpsolve->newbranchhandle = GetVariant(&lpsolve->lpsolvecaller, 3);
        put_bb_branchfunc(lpsolve->lp, branchfunction, lpsolve);
}


/* xxlpsolve('put_abortfunc', lp, newabort, aborthandle) */

static void impl_put_abortfunc(structlpsolve *lpsolve)
{
        Check_nrhs(lpsolve, 3);
        FreeFunction(lpsolve->newabort);
        FreeVariant(lpsolve->newaborthandle);
        lpsolve->newabort = GetFunction(&lpsolve->lpsolvecaller, 2);
        lpsolve->newaborthandle = GetVariant(&lpsolve->lpsolvecaller, 3);
        if (lpsolve->newabort == NULL)
                putabortfunc(lpsolve->lp, myabort, &lpsolve->lpsolvecaller);
        else
                put_abortfunc(lpsolve->lp, abortfunction, lpsolve);
}


/* xxlpsolve('put_logfunc', lp, newlog, loghandle) */

static void impl_put_logfunc(structlpsolve *lpsolve)
{
        Check_nrhs(lpsolve, 3);
        FreeFunction(lpsolve->newlog);
        FreeVariant(lpsolve->newloghandle);
        lpsolve->newlog = GetFunction(&lpsolve->lpsolvecaller, 2);
        lpsolve->newloghandle = GetVariant(&lpsolve->lpsolvecaller, 3);
        if (lpsolve->newlog == NULL)
                putlogfunc(lpsolve->lp, mylog, &lpsolve->lpsolvecaller);
        else
                put_logfunc(lpsolve->lp, logfunction, lpsolve);
}


/* xxlpsolve('put_msgfunc', lp, newmsg, msghandle, mask) */

static void impl_put_msgfunc(structlpsolve *lpsolve)
{
        int mask;

        Check_nrhs(lpsolve, 4);
        FreeFunction(lpsolve->newmsg);
        FreeVariant(lpsolve->newmsghandle);
        lpsolve->newmsg = GetFunction(&lpsolve->lpsolvecaller, 2);
        lpsolve->newmsghandle = GetVariant(&lpsolve->lpsolvecaller, 3);
        mask = (int) GetRealScalar(&lpsolve->lpsolvecaller, 4);
        put_msgfunc(lpsolve->lp, msgfunction, lpsolve, mask);
}
...
static struct {
  ...
  { "put_abortfunc", impl_put_abortfunc, TRUE },
  { "put_bb_branchfunc", impl_put_bb_branchfunc, TRUE },
  { "put_bb_nodefunc", impl_put_bb_nodefunc, TRUE },
  { "put_logfunc", impl_put_logfunc, TRUE },
  { "put_msgfunc", impl_put_msgfunc, TRUE },
...
#               endif
        	...
        	/* Now call the required part of the lp toolkit */
                ...
                if (routines[i].needshandle) {
                	...
                	if ((!handle_valid(lpsolve->h)) || ((lpsolve->lp = lpdata[lpsolve->h].lp) == NULL)) {
                        	strcpy(buf, lpsolve->cmd);
                        	strncat(buf, ": Invalid lp handle.", bufsz);
                		ErrMsgTxt(&lpsolve->lpsolvecaller, buf);
                	}
                        lpsolve->newbranch = lpdata[lpsolve->h].newbranch;
                        lpsolve->newbranchhandle = lpdata[lpsolve->h].newbranchhandle;
                        lpsolve->newnode = lpdata[lpsolve->h].newnode;
                        lpsolve->newnodehandle = lpdata[lpsolve->h].newnodehandle;
                        lpsolve->newabort = lpdata[lpsolve->h].newabort;
                        lpsolve->newaborthandle = lpdata[lpsolve->h].newaborthandle;
                        lpsolve->newlog = lpdata[lpsolve->h].newlog;
                        lpsolve->newloghandle = lpdata[lpsolve->h].newloghandle;
                        lpsolve->newmsg = lpdata[lpsolve->h].newmsg;
                        lpsolve->newmsghandle = lpdata[lpsolve->h].newmsghandle;
                }
                BEGIN_INTERRUPT_IMMEDIATELY_IN_FOREIGN_CODE;
                routines[i].routine(lpsolve);
                END_INTERRUPT_IMMEDIATELY_IN_FOREIGN_CODE;
                if (routines[i].needshandle) {
                        lpdata[lpsolve->h].newbranch = lpsolve->newbranch;
                        lpdata[lpsolve->h].newbranchhandle = lpsolve->newbranchhandle;
                        lpdata[lpsolve->h].newnode = lpsolve->newnode;
                        lpdata[lpsolve->h].newnodehandle = lpsolve->newnodehandle;
                        lpdata[lpsolve->h].newabort = lpsolve->newabort;
                        lpdata[lpsolve->h].newaborthandle = lpsolve->newaborthandle;
                        lpdata[lpsolve->h].newlog = lpsolve->newlog;
                        lpdata[lpsolve->h].newloghandle = lpsolve->newloghandle;
                        lpdata[lpsolve->h].newmsg = lpsolve->newmsg;
                        lpdata[lpsolve->h].newmsghandle = lpsolve->newmsghandle;
                }
        }
}
...
callerPrototype(drivername)
{
        ...
        lpsolve.newbranch = NULL;
        lpsolve.newbranchhandle = NULL;
        lpsolve.newnode = NULL;
        lpsolve.newnodehandle = NULL;
        lpsolve.newabort = NULL;
        lpsolve.newaborthandle = NULL;
        lpsolve.newlog = NULL;
        lpsolve.newloghandle = NULL;
        lpsolve.newmsg = NULL;
        lpsolve.newmsghandle = NULL;
